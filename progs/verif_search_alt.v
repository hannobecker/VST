...
Section SearchSpec.
  ...
  Proof.
    ...
    (* Need to discharge the assumption that
          search target lst = Some i *)
    move : H_eq.
...
Module C_search_verify (Imp : SearchImplementation ZTy).
  ...
  Proof.
    ...
    start_function.
    forward.
    (* Turns out that this loop invariant is significantly harder to prove... *)
    forward_while
      (EX i:Z,
         PROP  (0 <= i <= length;
...
(* An implementation of a search function for integers *)
