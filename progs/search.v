...
Definition _arr : ident := 49%positive.
Definition _cur_val : ident := 52%positive.
Definition _i : ident := 54%positive.
Definition _length : ident := 50%positive.
Definition _main : ident := 56%positive.
Definition _search : ident := 53%positive.
Definition _search_alt : ident := 55%positive.
Definition _target : ident := 51%positive.

Definition f_search := {|
  fn_return := (tptr tint);
  fn_callconv := cc_default;
  fn_params := ((_arr, (tptr tint)) :: (_length, tint) :: (_target, tint) ::
                nil);
  fn_vars := nil;
  fn_temps := ((_cur_val, tint) :: nil);
  fn_body :=
(Ssequence
  (Swhile
    (Ebinop Ogt (Etempvar _length tint) (Econst_int (Int.repr 0) tint) tint)
    (Ssequence
      (Sset _cur_val (Ederef (Etempvar _arr (tptr tint)) tint))
      (Ssequence
        (Sifthenelse (Ebinop Oeq (Etempvar _cur_val tint)
                       (Etempvar _target tint) tint)
          (Sreturn (Some (Etempvar _arr (tptr tint))))
          Sskip)
        (Ssequence
          (Sset _arr
...
Definition f_search_alt := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_arr, (tptr tint)) :: (_length, tint) :: (_target, tint) ::
                nil);
  fn_vars := nil;
  fn_temps := ((_i, tint) :: (_cur_val, tint) :: nil);
  fn_body :=
(Ssequence
  (Sset _i (Econst_int (Int.repr 0) tint))
  (Ssequence
    (Swhile
      (Ebinop Olt (Etempvar _i tint) (Etempvar _length tint) tint)
      (Ssequence
        (Sset _cur_val
          (Ederef
...
((___builtin_fabs,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_search, Gfun(Internal f_search)) ::
 (_search_alt, Gfun(Internal f_search_alt)) :: nil);
prog_public :=
(_search_alt :: _search :: ___builtin_debug :: ___builtin_nop ::
